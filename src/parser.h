 ...
 *                entry point.
 *
 * As of Stage 3 the input_file / output_file / append_mode fields are live
 * (redirection); background stays unused until Stage 5. Declaring them all up
 * front means the executor's structs never reshape as features land.
 */
 ...
 *             including any input_file/output_file redirection targets.
 *
 * Returns the number of arguments parsed (cmd->argc); 0 means the segment was
 * blank or whitespace-only; -1 means a syntax error (e.g. a redirection
 * operator with no following filename), already reported to stderr.
 */
 ...
 * blank; -1 means a segment had a syntax error (already reported). Empty
 * segments (e.g. a stray "ls | | grep") are skipped rather than treated as a
 * syntax error — a deliberate Stage 2 simplification.
 */
